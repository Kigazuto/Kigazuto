# 👋 Pengyu Jing

🤖 I work on **robot learning and manipulation**

## 🔬 Research Interests

- 🦾 Robot Learning and Robotic Manipulation
- 🧠 Vision-Language-Action Models
- 🛠️ Simulation and Real-Robot Systems

## 💻 Technical Background

- **Programming:** Python, C++, MATLAB
- **Robot Learning:** PyTorch, PPO, DSRL, VLA, π0
- **Robotics:** ROS/ROS 2, MuJoCo, Isaac Sim, PyBullet
- **Hardware:** STM32, CAN, PCB Design, Embedded Systems

## 📚 Selected Research

- 🤖 Real-world RL for dual-arm robotic manipulation
- 👕 Simulator-in-the-loop refinement for cloth manipulation
- 🦿 Reinforcement learning and model-based control for robotic systems
- 🔧 Embedded control and custom electronics for Robocon robots

## 🔗 Links

- 🎓 [Google Scholar](https://scholar.google.com/citations?user=y0yKWaYAAAAJ)
- 💼 [LinkedIn](https://www.linkedin.com/in/pengyu-jing-63a065399/)
- 𝕏 [X](https://x.com/jing_pengy68112)
- ✉️ [Email](mailto:jingpengyu22@gmail.com)
